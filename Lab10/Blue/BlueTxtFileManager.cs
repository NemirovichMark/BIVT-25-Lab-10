...
public class BlueTxtFileManager<T> : BlueFileManager<T> where T:Lab9.Blue.Blue
{
  ...
  public BlueTxtFileManager(string name, string folderpath, string filename, string ext = "") : base(name, folderpath, filename, ext){}
  ...
  public override void Serialize(T obj)
  {
    ...
    using (var writer = new StreamWriter(FullPath))
    {
      ...
      {
	writer.WriteLine($"Output: {task2.Output}");
      }
      writer.WriteLine($"Type: {obj.GetType().AssemblyQualifiedName}");
      writer.WriteLine($"Input: {obj.Input}");
      if (obj is Lab9.Blue.Task2 task_2)
	writer.WriteLine($"Substr: {task_2.Substr}");
    }
  }
  public override T Deserialize()
  {
    ...
    if (dict.ContainsKey("Type"))
    {
      ...
      if (dict.ContainsKey("Substr")) txt_sub = dict["Substr"];
    }
    ...
    {
      obj = new Lab9.Blue.Task2(txt_input, txt_sub);
    }
